...
correctly formatted template is downloadable from within the application.
...
verified directly: the four template rows score 1.4\%, 11.7\%, 36.0\% and
22.3\%, exactly matching the four presets on the single-order form.
...
\projfig{fig47_hf_dataset_repo}{0.98}{The dataset repository after stages 1 and
2. Each file arrives as its own commit, giving the data a version history.}

\projfig{fig48_hf_model_repo}{0.98}{The model registry. The commit message
records the algorithm, its ROC-AUC and the decision threshold, so a deployed
model can be identified without opening it.}
...
\projfig{fig50_mlflow_ui}{0.98}{The MLflow tracking interface, opened locally
from the artefact produced by a pipeline run.}
...
Eight substantive problems arose during development. Each is recorded with its
diagnosis and resolution, because the resolutions are more instructive than a
narrative in which nothing went wrong.
...
resolution, metadata validation, four incomplete dependency declarations, and
credential hygiene. Not one of them was a modelling problem.
